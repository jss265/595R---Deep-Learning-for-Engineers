...
This code uses an autoencoder to compress monthly weather samples into a latent space to be
solved by an ODE Solver and then decoded into usable prediction data.
...
train_size = 20
ae_sizes = [4, 8, 4, 2]      # AE input->hidden->latent
latent_sizes = [2, 16, 2]    # ODE network latent sizes
activation = nn.ReLU
lr = 0.01
stage_size = 4
max_epochs = 100
...
class NeuralODE(nn.Module):
    ...
    def solveODE(self, t, y):
        return self.network(y)
    def forward(self, y0, tsteps):
        return odeint(self.solveODE, y0, tsteps)
...
class NeuralODEAutoencoder(nn.Module):
    ...
    def forward(self, x, tsteps):
        ...
        xhat = self.decoder(zt)   # keep all timesteps
        return xhat
...
while current_end <= X_train.shape[0]:
    ...
    for epoch in range(max_epochs):
        ...
        xhat_last = xhat[-1]               # only last timestep for loss
        loss = loss_fn(xhat_last, x_stage)
        loss.backward()
        optimizer.step()
    ...
    # ------- Plot after each stage -------
    with torch.no_grad():
        xhat_stage = model(x_stage, tsteps).numpy()
        x_stage_np = x_stage.numpy()
    ...
    for i in range(4):
        ...
        axs[i].plot(xhat_stage[:, i], '--', label='Pred')  # full sequence
